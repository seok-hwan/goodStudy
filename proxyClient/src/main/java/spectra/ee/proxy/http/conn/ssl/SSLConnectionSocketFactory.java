...
package spectra.ee.proxy.http.conn.ssl;
 ...
 * <p>
 * SSLSocketFactory can be used to validate the identity of the HTTPS server against a list of trusted certificates and
 * to authenticate to the HTTPS server using a private key.
 * <p>
 * SSLSocketFactory will enable server authentication when supplied with a {@link java.security.KeyStore trust-store}
 * file containing one or several trusted certificates. The client secure socket will reject the connection during the
 * SSL session handshake if the target HTTPS server attempts to authenticate itself with a non-trusted certificate.
 * <p>
 ...
 * <p>
 * SSLSocketFactory will enable client authentication when supplied with a {@link java.security.KeyStore key-store} file
 * containing a private key/public certificate pair. The client secure socket will use the private key to authenticate
...
public class SSLConnectionSocketFactory implements LayeredConnectionSocketFactory
{
     ...
     * <code>cacerts</code> file in the security properties directory). System properties are not taken into
     * consideration.
     *
    ...
    /**
     * Obtains default SSL socket factory with an SSL context based on system properties as described in <a
     * href="http://docs.oracle.com/javase/1.5.0/docs/guide/security/jsse/JSSERefGuide.html"> "JavaTM Secure Socket
     * Extension (JSSE) Reference Guide for the JavaTM 2 Platform Standard Edition 5</a>
     *
    ...
    /**
     * Performs any custom initialization for a newly created SSLSocket (before the SSL handshake happens). The default
     * implementation is a no-op, but could be overridden to, e.g., call

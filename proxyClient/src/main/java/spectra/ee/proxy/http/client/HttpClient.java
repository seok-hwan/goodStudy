...
package spectra.ee.proxy.http.client;
...
/**
 * This interface represents only the most basic contract for HTTP request execution. It imposes no restrictions or
 * particular details on the request execution process and leaves the specifics of state management, authentication and
 * redirect handling up to individual implementations.
 *
 * @since 4.0
 */
public interface HttpClient
{
     ...
     *
     * @param request the request to execute
     * @param context the context to use for the execution, or <code>null</code> to use the default context
     * @return the response to the request. This is always a final response, never an intermediate response with an 1xx
     *         status code. Whether redirects or authentication challenges will be returned or handled automatically
     *         depends on the implementation and configuration of this client.
     * @throws IOException in case of a problem or the connection was aborted
     ...
     * Executes HTTP request using the default context.
     *
     * @param target the target host for the request. Implementations may accept <code>null</code> if they can still
     ...
     * <p/>
     * Implementing classes are required to ensure that the content entity associated with the response is fully
     * consumed and the underlying connection is released back to the connection manager automatically in all cases
     * relieving individual {@link ResponseHandler}s from having to manage resource deallocation internally.
     *
     * @param request the request to execute
    ...
    /**
     * Executes HTTP request to the target using the default context and processes the response using the given response
     * handler.
     * <p/>
     * Implementing classes are required to ensure that the content entity associated with the response is fully
     * consumed and the underlying connection is released back to the connection manager automatically in all cases
    ...
    /**
     * Executes HTTP request to the target using the given context and processes the response using the given response
     * handler.
     * <p/>
     * Implementing classes are required to ensure that the content entity associated with the response is fully
     * consumed and the underlying connection is released back to the connection manager automatically in all cases

...
package spectra.ee.proxy.http.protocol;
...
import spectra.ee.proxy.http.HttpVersion;
...
/**
 * <tt>HttpRequestExecutor</tt> is a client side HTTP protocol handler based on the blocking (classic) I/O model.
 * <p/>
 * <tt>HttpRequestExecutor</tt> relies on {@link HttpProcessor} to generate mandatory protocol headers for all outgoing
 * messages and apply common, cross-cutting message transformations to all incoming and outgoing messages. Application
 * specific processing can be implemented outside <tt>HttpRequestExecutor</tt> once the request has been executed and a
...
public class HttpRequestExecutor
{
    ...
    /**
     * Decide whether a response comes with an entity. The implementation in this class is based on RFC 2616. <br/>
     ...
     * <p>
     * This method also handles the expect-continue handshake if necessary. If it does not have to handle an
     * expect-continue handshake, it will not use the connection for reading or anything else that depends on data
    ...
    /**
     * Waits for and receives a response. This method will automatically ignore intermediate responses with status code
     * 1xx.
     *
    ...
    /**
     * Post-processes the given response using the given protocol processor and completes the process of request
     * execution.
     * <p>
     * This method does <i>not</i> read the response entity, if any. The connection over which content of the response
     * entity is being streamed from cannot be reused until
     * {@link org.apache.http.util.EntityUtils#consume(org.apache.http.HttpEntity)} has been invoked.
     *

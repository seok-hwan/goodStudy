...
import spectra.ee.proxy.http.config.Registry;
import spectra.ee.proxy.http.config.RegistryBuilder;
import spectra.ee.proxy.http.config.SocketConfig;
import spectra.ee.proxy.http.conn.ConnectionPoolTimeoutException;
...
import spectra.ee.proxy.http.conn.HttpConnectionFactory;
...
import spectra.ee.proxy.http.pool.ConnFactory;
import spectra.ee.proxy.http.pool.ConnPoolControl;
import spectra.ee.proxy.http.pool.PoolStats;
...
/**
 * <tt>ClientConnectionPoolManager</tt> maintains a pool of {@link HttpClientConnection}s and is able to service
 * connection requests from multiple execution threads. Connections are pooled on a per route basis. A request for a
 * route which already the manager has persistent connections for available in the pool will be services by leasing a
 * connection from the pool rather than creating a brand new connection.
 * <p/>
 * <tt>ClientConnectionPoolManager</tt> maintains a maximum limit of connection on a per route basis and in total. Per
 * default this implementation will create no more than than 2 concurrent connections per given route and no more 20
 * connections in total. For many real-world applications these limits may prove too constraining, especially if they
 * use HTTP as a transport protocol for their services. Connection limits, however, can be adjusted using
 * {@link ConnPoolControl} methods.
 *
 * @since 4.3
 */
@ThreadSafe
public class PoolingHttpClientConnectionManager implements HttpClientConnectionManager, ConnPoolControl<HttpRoute>, Closeable
{
    ...
    public ConnectionRequest requestConnection(final HttpRoute route, final Object state)
    {
        ...
        {
            logger.debug("Connection request: " + format(route, state) + formatStats(route));
        }
    ...
    protected HttpClientConnection leaseConnection(final Future<CPoolEntry> future, final long timeout, final TimeUnit tunit) throws InterruptedException, ExecutionException, ConnectionPoolTimeoutException
    {
        ...
        try
        {
            ...
            if (logger.isDebugEnabled())
            {
                logger.debug("Connection leased: " + format(entry) + formatStats(entry.getRoute()));
            }
    ...
    public void releaseConnection(final HttpClientConnection managedConn, final Object state, final long keepalive, final TimeUnit tunit)
    {
        ...
        synchronized (managedConn)
        {
            ...
            try
            {
                if (conn.isOpen())
                {
                    ...
                    if (logger.isDebugEnabled())
                    {
                        ...
                        logger.debug("Connection " + format(entry) + " can be kept alive " + s);
                    }
                }
            }
            finally
            {
                ...
                if (logger.isDebugEnabled())
                {
                    logger.debug("Connection released: " + format(entry) + formatStats(entry.getRoute()));
                }
            }
        }
    }
    ...
    public void shutdown()
    {
        ...
        {
            logger.debug("Connection manager is shutting down");
            try
            {
                this.pool.shutdown();
            }
            ...
            {
                logger.debug("I/O exception shutting down connection manager", ex);
            }
    ...
    public void closeIdleConnections(final long idleTimeout, final TimeUnit tunit)
    {
        if (logger.isDebugEnabled())
        {
            logger.debug("Closing connections idle longer than " + idleTimeout + " " + tunit);
        }

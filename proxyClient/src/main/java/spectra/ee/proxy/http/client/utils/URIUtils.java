 ...
 *
 */
package spectra.ee.proxy.http.client.utils;
...
public class URIUtils
{
    ...
    /**
     * Constructs a {@link URI} using all the parameters. This should be used instead of
     * {@link URI#URI(String, String, String, int, String, String, String)} or any of the other URI multi-argument URI
     * constructors.
     *
     ...
     * @throws URISyntaxException If both a scheme and a path are given but the path is relative, if the URI string
     *             constructed from the given components violates RFC&nbsp;2396, or if the authority component of the
     *             string is present but cannot be parsed as a server-based authority
    ...
    /**
     * A convenience method for creating a new {@link URI} whose scheme, host and port are taken from the target host,
     * but whose path, query and fragment are taken from the existing URI. The fragment is only used if dropFragment is
     * false. The path is set to "/" if not explicitly specified.
     *
    ...
    /**
     * A convenience method that creates a new {@link URI} whose scheme, host, port, path, query are taken from the
     * existing URI, dropping any fragment or user-information. The path is set to "/" if not explicitly specified. The
     * existing URI is returned unmodified if it has no fragment or user-information and has a path.
     *
    ...
    /**
     * Removes dot segments according to RFC 3986, section 5.2.4 and Syntax-Based Normalization according to RFC 3986,
     * section 6.2.2.
     *
    ...
    /**
     * Derives the interpreted (absolute) URI that was used to generate the last request. This is done by extracting the
     * request-uri and target origin for the last request and scanning all the redirect locations for the last fragment
     * identifier, then combining the result into a {@link URI}.
     *
     * @param originalURI original request before any redirects
     * @param target if the last URI is relative, it is resolved against this target, or <code>null</code> if not
     *            available.
     * @param redirects collection of redirect locations since the original request or <code>null</code> if not
     *            available.

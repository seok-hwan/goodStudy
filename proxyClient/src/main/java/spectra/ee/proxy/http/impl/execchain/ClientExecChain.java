...
/**
 * This interface represents an element in the HTTP request execution chain. Each element can either be a decorator
 * around another element that implements a cross cutting aspect or a self-contained executor capable of producing a
 * response for the given request.
 * <p/>
 * Important: please note it is required for decorators that implement post execution aspects or response
 * post-processing of any sort to release resources associated with the response by calling
 * {@link CloseableHttpResponse#close()} methods in case of an I/O, protocol or runtime exception, or in case the
 * response is not propagated to the caller.
 *
 * @since 4.3
 */
public interface ClientExecChain
{
    ...
    /**
     * Executes th request either by transmitting it to the target server or by passing it onto the next executor in the
     ...
     * @return HTTP response either received from the opposite endpoint or generated locally.

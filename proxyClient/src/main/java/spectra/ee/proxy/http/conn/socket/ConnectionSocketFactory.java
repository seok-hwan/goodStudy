...
package spectra.ee.proxy.http.conn.socket;
...
public interface ConnectionSocketFactory
{
     ...
     * {@link #connectSocket(int, Socket, HttpHost, InetSocketAddress, InetSocketAddress, HttpContext) connectSocket}
     * method.
     *
     * @return a new socket
     ...
     * @param sock the socket to connect, as obtained from {@link #createSocket(HttpContext)}. <code>null</code>
     ...
     * @return the connected socket. The returned object may be different from the <code>sock</code> argument if this
     *         factory supports a layered protocol.

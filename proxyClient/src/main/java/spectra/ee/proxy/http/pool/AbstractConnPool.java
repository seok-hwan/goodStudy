 ...
 *
 */
package spectra.ee.proxy.http.pool;

import org.apache.log4j.Logger;
import spectra.ee.proxy.http.annotation.ThreadSafe;
import spectra.ee.proxy.http.concurrent.FutureCallback;
 ...
 * <p/>
 * Please note that this class does not maintain its own pool of execution {@link Thread}s. Therefore, one <b>must</b>
 * call {@link Future#get()} or {@link Future#get(long, TimeUnit)} method on the {@link Future} object returned by the
...
public abstract class AbstractConnPool<T, C, E extends PoolEntry<T, C>> implements ConnPool<T, E>, ConnPoolControl<T>
{
     ...
     * {@inheritDoc}
     * <p/>
     * Please note that this class does not maintain its own pool of execution {@link Thread}s. Therefore, one
     * <b>must</b> call {@link Future#get()} or {@link Future#get(long, TimeUnit)} method on the {@link Future} returned
     * by this method in order for the lease operation to complete.
     */
     ...
     * @param state arbitrary object that represents a particular state (usually a security principal or a unique token
     *            identifying the user whose credentials have been used while establishing the connection). May be
     *            <code>null</code>.
    ...
    private E getPoolEntryBlocking(final T route, final Object state, final long timeout, final TimeUnit tunit, final PoolEntryFuture<E> future) throws IOException, InterruptedException, TimeoutException
    {
        ...
        if (timeout > 0)
        {
            ...
            logger.debug("deadline time : " + deadline);
        }

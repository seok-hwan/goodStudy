...
/**
 * Provides access to version information for HTTP components. Static methods are used to extract version information
 * from property files that are automatically packaged with HTTP component release JARs. <br/>
 * All available version information is provided in strings, where the string format is informal and subject to change
 * without notice. Version information is provided for debugging output and interpretation by humans, not for automated
...
public class VersionInfo
{
    ...
    /**
     * Obtains the name of the versioned module or informal unit. This data is read from the version information for the
     * package.
     *
    ...
    /**
     * Obtains the release of the versioned module or informal unit. This data is read from the version information for
     * the package.
     *
    ...
    /**
     * Obtains the timestamp of the versioned module or informal unit. This data is read from the version information
     * for the package.
     *
    ...
    /**
     * Obtains the classloader used to read the version information. This is just the <code>toString</code> output of
     * the classloader, since the version information should not keep a reference to the classloader itself. That could
    ...
    {
        final StringBuilder sb = new StringBuilder(20 + infoPackage.length() + infoModule.length() + infoRelease.length() + infoTimestamp.length() + infoClassloader.length());
     ...
     * @param clsldr the classloader to load from, or <code>null</code> for the thread context classloader
     * @return the version information for all packages found, never <code>null</code>
     */
     ...
     *
     * @param pckg the package for which to load version information, for example "org.apache.http". The package name
     *            should NOT end with a dot.
     * @param clsldr the classloader to load from, or <code>null</code> for the thread context classloader
     * @return the version information for the argument package, or <code>null</code> if not available
     */
     ...
     * @param info the map from string keys to string values, for example {@link java.util.Properties}
     ...
     * @param pkg the package for which to load version information, for example "org.apache.http". The package name
     *            should NOT end with a dot.
     * @param cls the class' class loader to load from, or <code>null</code> for the thread context class loader
     * @since 4.3
     */

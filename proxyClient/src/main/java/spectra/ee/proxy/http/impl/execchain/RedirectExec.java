...
import spectra.ee.proxy.http.client.RedirectException;
        ...
        for (int redirectCount = 0;;)
        {
            ...
            try
            {
                if (config.isRedirectsEnabled() && this.redirectStrategy.isRedirected(currentRequest, response, context))
                {
                    ...
                    {
                        logger.debug("Redirecting to '" + uri + "' via " + currentRoute);
                    }
            ...
            catch (final HttpException ex)
            {
                ...
                {
                    logger.debug("I/O error while releasing connection", ioex);
                }
                finally
                {
                    response.close();

...
class LoggingManagedHttpClientConnection extends DefaultManagedHttpClientConnection
{
    ...
    public void close() throws IOException
    {
        ...
        {
            logger.debug(getId() + ": Close connection");
        }
        super.close();
    }
    ...
    public void shutdown() throws IOException
    {
        if (logger.isDebugEnabled())
        {
            logger.debug(getId() + ": Shutdown connection");
        }
        super.shutdown();
    }
    ...
    protected void onResponseReceived(final HttpResponse response)
    {
        ...
        {
            logger.debug(getId() + " << " + response.getStatusLine().toString());
            final Header[] headers = response.getAllHeaders();
            for (final Header header : headers)
            {
                logger.debug(getId() + " << " + header.toString());
            }
        }
    }
    ...
    protected void onRequestSubmitted(final HttpRequest request)
    {
        ...
        {
            logger.debug(getId() + " >> " + request.getRequestLine().toString());

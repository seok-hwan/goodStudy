...
import spectra.ee.proxy.http.ConnectionClosedException;
...
/**
 * Input stream that cuts off after a defined number of bytes. This class is used to receive content of HTTP messages
 * where the end of the content entity is determined by the value of the <code>Content-Length header</code>. Entities
 * transferred using this stream can be maximum {@link Long#MAX_VALUE} long.
 * <p>
 * Note that this class NEVER closes the underlying stream, even when close gets called. Instead, it will read until the
 * "end" of its limit on close, which allows for the seamless execution of subsequent HTTP 1.1 requests, while not

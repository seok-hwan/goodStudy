...
/**
 * Interface for parsing header values into elements. Instances of this interface are expected to be stateless and
 * thread-safe.
 *
 * @since 4.0
 */
public interface HeaderValueParser
{
    ...
    /**
     * Parses a header value into elements. Parse errors are indicated as <code>RuntimeException</code>.
     * <p>
     * Some HTTP headers (such as the set-cookie header) have values that can be decomposed into multiple elements. In
     * order to be processed by this parser, such headers must be in the following form:
     * </p>
     * 
     * <pre>
     ...
     * </pre>
     * <p>
     * Any amount of white space is allowed between any part of the header, element or param and is ignored. A missing
     * value in any element or param will be stored as the empty {@link String}; if the "=" is also missing
    ...
    /**
     * Parses a list of name-value pairs. These lists are used to specify parameters to a header element. Parse errors
     * are indicated as <code>ParseException</code>.
     *
     ...
     * @param cursor the parser cursor containing the current position and the bounds within the buffer for the parsing
     *            operation
     * @return the name-value pair, where the value is <code>null</code> if no value is specified
     */

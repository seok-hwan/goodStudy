...
package spectra.ee.proxy.http.entity.mime;
...
import spectra.ee.proxy.http.entity.mime.content.ContentBody;
import spectra.ee.proxy.http.util.Args;
import spectra.ee.proxy.http.util.ByteArrayBuffer;

/**
 * HttpMultipart represents a collection of MIME multipart encoded content bodies. This class is capable of operating
 * either in the strict (RFC 822, RFC 2045, RFC 2046 compliant) or the browser compatible modes.
 *
 * @since 4.3
 */
abstract class AbstractMultipartForm
{
     ...
     * @param charset the character set to use. May be {@code null}, in which case {@link MIME#DEFAULT_CHARSET} - i.e.
     *            US-ASCII - is used.
     ...
     */
    protected abstract void formatMultipartHeader(final FormBodyPart part, final OutputStream out) throws IOException;

    /**
     * Writes out the content in the multipart/form encoding. This method produces slightly different formatting
     * depending on its compatibility mode.
     */
    ...
    /**
     * Determines the total length of the multipart content (content length of individual parts plus that of extra
     * elements required to delimit the parts from one another). If any of the @{link BodyPart}s contained in this
     * object is of a streaming entity of unknown length the total length is also unknown.
     * <p/>
     * This method buffers only a small amount of data in order to determine the total length of the entire entity. The
     * content of individual parts is not buffered.
     *
     * @return total length of the multipart entity if known, <code>-1</code> otherwise.
     */

...
import spectra.ee.proxy.http.HttpResponseFactory;
...
import spectra.ee.proxy.http.impl.DefaultHttpResponseFactory;
import spectra.ee.proxy.http.impl.io.AbstractMessageParser;
...
/**
 * Lenient HTTP response parser implementation that can skip malformed data until a valid HTTP response message head is
 * encountered.
 *
 * @since 4.2
 */
...
public class DefaultHttpResponseParser extends AbstractMessageParser<HttpResponse>
{
     ...
     * @param lineParser the line parser. If <code>null</code> {@link org.apache.http.message.BasicLineParser#INSTANCE}
     *            will be used.
     * @param responseFactory HTTP response factory. If <code>null</code> {@link DefaultHttpResponseFactory#INSTANCE}
     *            will be used.
    ...
    protected HttpResponse parseHead(final SessionInputBuffer sessionBuffer) throws IOException, HttpException
    {
        ...
        do
        {
            ...
            {
                logger.debug("Garbage in response: " + this.lineBuf.toString());
            }
            count++;
        }
        while (true);

...
import spectra.ee.proxy.http.io.BufferInfo;
import spectra.ee.proxy.http.io.HttpTransportMetrics;
...
import spectra.ee.proxy.http.util.ByteArrayBuffer;
import spectra.ee.proxy.http.util.CharArrayBuffer;

/**
 * Abstract base class for session input buffers that stream data from an arbitrary {@link InputStream}. This class
 * buffers input data in an internal byte array for optimal input performance.
 * <p/>
 * {@link #readLine(CharArrayBuffer)} and {@link #readLine()} methods of this class treat a lone LF as valid line
...
public class SessionInputBufferImpl implements SessionInputBuffer, BufferInfo
{
     ...
     * @param minChunkLimit size limit below which data chunks should be buffered in memory in order to minimize native
     *            method invocations on the underlying network socket. The optimal value of this parameter can be
     *            platform specific and defines a trade-off between performance of memory copy operations and that of
     *            native method invocation. If negative default chunk limited will be used.
     * @param constraints Message constraints. If <code>null</code> {@link MessageConstraints#DEFAULT} will be used.
     * @param chardecoder chardecoder to be used for decoding HTTP protocol elements. If <code>null</code> simple type
     *            cast will be used for byte to char conversion.
     */
     ...
     * The number of chars actually read is returned as an integer. The line delimiter itself is discarded. If no char
     * is available because the end of the stream has been reached, the value <code>-1</code> is returned. This method
    ...
    /**
     * Reads a complete line of characters up to a line delimiter from this session buffer. The line delimiter itself is
     * discarded. If no char is available because the end of the stream has been reached, <code>null</code> is returned.

...
public class ProxySimpleServlet extends ProxyBaseServlet
{
    /**
     * Serializable UID
     */
    private static final long serialVersionUID = 5539400208154020618L;
//    private static final String PARAM_CMD = "cmd";
	...
	protected void processAfterSetParamMap(HttpServletRequest req, HttpServletResponse res, HashMap dataMap, boolean isMultipart) 
	{
        ...
        {
            paramMap = (HashMap)paramMap.get(ProxyHttpClient.REQUEST_PARAM_CMD);
        }
	 ...
	 * @param isMultipart
	 */
	protected boolean processAfterCallApi(HttpServletRequest req, HttpServletResponse res, HashMap dataMap, boolean isMultipart) 
	{
		try 
        { 
		    if (dataMap==null)
		    {   
		        // 이미 상위에서 에러 발생시 로그를 찍기 때문에 별도로 찍지 않는다.
		        return false;
		    }
